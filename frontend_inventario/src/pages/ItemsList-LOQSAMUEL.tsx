      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-200 mb-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-bold text-slate-700"
                />
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Categorías</label>
              <select
                value={selectedCategory || ''}
                onChange={(e) => setSelectedCategory(e.target.value ? Number(e.target.value) : null)}
                className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 font-bold text-slate-700 appearance-none cursor-pointer"
              >
            ...
            {/* Filtro de Stock (BOTÓN FORZADO A BLANCO) */}
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Estado de Alerta</label>
              <button
                onClick={() => setShowLowStockOnly(!showLowStockOnly)}
                style={{ backgroundColor: 'white', color: showLowStockOnly ? '#dc2626' : '#475569', borderColor: showLowStockOnly ? '#dc2626' : '#e2e8f0' }}
                className="w-full py-4 rounded-2xl font-black text-sm uppercase tracking-tight transition-all border-2 flex items-center justify-center gap-2 shadow-sm"
              >
                {showLowStockOnly ? '✓ Viendo Bajo Stock' : 'Ver solo Bajo Stock'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredItems.map((item) => (
            <div key={item.id} className="bg-white rounded-3xl p-5 border border-slate-200 hover:shadow-xl transition-all duration-300">
              <div className="flex flex-col lg:flex-row lg:items-center gap-6">
                ...
                {/* Stock */}
                <div className="flex flex-row lg:flex-col items-center justify-center bg-slate-50 px-6 py-2 rounded-2xl border border-slate-100">
                  <span className="text-2xl font-black text-slate-900">{item.currentStock}</span>
                  <span className="text-[8px] font-black text-slate-400 uppercase lg:ml-0 ml-2">Stock</span>
                </div>
                ...
                {/* Acciones (BOTÓN ELIMINAR FORZADO A BLANCO) */}
                <div className="flex items-center justify-end gap-2 border-t lg:border-t-0 pt-4 lg:pt-0">
                  <Link
                    to={`/items/${item.id}`}
                    className="flex-1 lg:flex-none px-4 py-3 bg-white text-slate-600 rounded-xl border border-slate-200 hover:border-indigo-600 transition-all text-center font-black text-xs uppercase"
                  >
                    Ver
                  </Link>
                  <Link
                    to={`/items/${item.id}/edit`}
                    className="flex-1 lg:flex-none px-4 py-3 bg-white text-slate-600 rounded-xl border border-slate-200 hover:border-blue-600 transition-all text-center font-black text-xs uppercase"
                  >
                    Editar
                  </Link>
                  <button
                    ...
                    className="flex-1 lg:flex-none px-4 py-3 rounded-xl border hover:border-red-600 transition-all text-center font-black text-xs uppercase"
                  >
                    Eliminar
                  </button>
                </div>
        ...
        {filteredItems.length === 0 && (
          <div className="bg-white rounded-[3rem] p-20 text-center border border-slate-200">
             ...
             <button 
                ...
                className="mt-6 px-10 py-4 border-2 rounded-2xl font-black text-xs uppercase transition-all shadow-sm"
             >
